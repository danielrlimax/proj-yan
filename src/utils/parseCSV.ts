    ...
    Papa.parse<string[]>(text, {
      ...
      complete: (results) => {
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          // Se as taxas não estiverem preenchidas nas colunas individuais do produto (L a N), 
          // ele herda de forma segura as globais (A a C).
          const mktStr = (row[11] || row[0] || '').trim();
